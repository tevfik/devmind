...
# Task Decomposition
# ============================================================================
def decompose_task_with_llm(user_request: str, context: Optional[Dict] = None) -> TaskDecomposition:
...
def create_tasks_from_decomposition(decomposition: TaskDecomposition) -> List[Task]:
    ...
    for i, subtask_desc in enumerate(decomposition.subtasks):
        ...
        priority = TaskPriority(priority_str) if priority_str in ['critical', 'high', 'medium', 'low'] else TaskPriority.MEDIUM
    ...
    for subtask_desc, deps in decomposition.dependencies.items():
        if subtask_desc in subtask_ids:
            ...
            if task:
                task.dependencies = [subtask_ids.get(dep, "") for dep in deps if dep in subtask_ids]
...
def execute_task_with_llm(task: Task, context: Dict) -> Dict[str, any]:
    ...
    if context.get('file_analyses'):
        ...
        relevant_analyses = [a for a in analyses if any(f in task.title or f in task.description for f in [a.file_path, a.file_path.split('/')[-1]])]
        if relevant_analyses:
            context_str += "\n\nRelevant Files Analysis:\n" + "\n".join([f"- {a.file_path}: {a.suggestions}" for a in relevant_analyses])
...
def update_task_status(tasks: List[Task], task_id: str, status: TaskStatus, result: Optional[str] = None, error: Optional[str] = None) -> List[Task]:
    """Update task status"""
    for task in tasks:
        if task.id == task_id:
            task.status = status
...
def run_iteration_cycle(state: DevMindState) -> dict:
    ...
    if iteration_count >= config.task.max_iterations:
        ...
        return {
            ...
            'log': state.get('log', []) + [format_log_entry('TaskManager', 'Max iterations reached')]
        }
    ...
    if not next_task:
        ...
        return {
            'should_continue': False,
            'log': state.get('log', []) + [format_log_entry('TaskManager', 'No more tasks to execute')]
        }
    ...
    return {
        ...
        'completed_tasks': state.get('completed_tasks', []) + ([next_task.id] if execution_result['success'] else []),
        'should_continue': should_continue,
        'log': state.get('log', []) + [
            format_log_entry('TaskManager', f"Iteration {iteration_count + 1}: Executed task {next_task.id}"),
            format_log_entry('TaskManager', f"Status: {'✅ Success' if execution_result['success'] else '❌ Failed'}")
        ]
    }
    ...
    if not existing_tasks:
        ...
        return {
            ...
            'log': state.get('log', []) + [
                ...
                format_log_entry('TaskManager', f"Complexity: {decomposition.estimated_complexity}")
            ]
        }
    else:
        # Run iteration
...
def execute_specific_task(state: DevMindState, task_data: dict) -> dict:
    ...
    if result['success']:
        ...
        if hasattr(task, 'repo_path') and task.repo_path:
            ...
            repo_path = '.'
            logger.warning(f"No repo_path found in task or state, using current directory: {repo_path}")
...
if __name__ == "__main__":
    ...
    test_request = "Analyze the codebase and create a refactoring plan for high-complexity files"
