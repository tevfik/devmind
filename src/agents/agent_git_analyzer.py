    ...
    try:
        ...
        for func in analysis.function_list:
            ...
            if func.cyclomatic_complexity > 10:
                complex_functions.append(f"{func.name} (complexity: {func.cyclomatic_complexity})")
    ...
    if language == 'Python':
        ...
        if stats:
            ...
            if stats['complex_functions']:
                analysis.suggestions.append(f"Refactor complex functions: {', '.join(stats['complex_functions'])}")
    else:
...
def analyze_repository_files(repo: git.Repo) -> Tuple[List[FileAnalysis], Dict[str, int]]:
...
def get_repository_info(repo: git.Repo) -> RepositoryInfo:
    ...
    try:
        ...
        return RepositoryInfo(repo_path=".", repo_url="", branch="unknown", last_commit="", contributors=[])

import json
from langchain_core.messages import SystemMessage, HumanMessage

def analyze_architecture(repo_path: Path, file_analyses: List[FileAnalysis], code_structure: Optional[List[Dict]] = None, user_request: str = "", mermaid_graph: Optional[str] = None) -> ArchitectureAnalysis:
    ...
    complex_files = sorted(file_analyses, key=lambda f: f.complexity or 0, reverse=True)[:10]
    complex_files_summary = "\n".join([f"- {f.file_path} (Complexity: {f.complexity}, Lang: {f.language})" for f in complex_files])
    ...
    if user_request:
        for f in file_analyses:
            if Path(f.file_path).name in user_request or Path(f.file_path).name.lower() in user_request.lower():
        ...
        try:
            ...
            if full_path.exists():
                with open(full_path, 'r', encoding='utf-8', errors='ignore') as file:
                    ...
                    file_contents.append(f"--- File: {f.file_path} ---\n{content}\n...\n")
        except Exception as e:
    ...
    try:
        response = llm.invoke([
            SystemMessage(content="You are a helpful software architect. Return only valid JSON."),
...
def git_analyzer_node(state: DevMindState) -> Dict:
    ...
    if not repo:
        ...
        if os.path.isdir(repo_path):
            class MockRepo:
                ...
                def rglob(self, p): return Path(repo_path).rglob(p)
            repo = MockRepo()
        else:
    ...
    if isinstance(repo, git.Repo):
        ...
        repo_info = RepositoryInfo(repo_path=str(repo_path), repo_url="", branch="", last_commit="", contributors=[])
    ...
    analysis_results = {
        'repo_info': repo_info,
        'file_analyses': file_analyses,
        'code_quality_score': round(sum(f.maintainability for f in file_analyses) / len(file_analyses), 2) if file_analyses else 0.0,
