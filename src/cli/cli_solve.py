...
def handle_solve(args):
    ...
    while outer_loop_count < MAX_OUTER_LOOP:
        ...
        console.print(Panel(Markdown(plan), title=f"Execution Plan (Attempt {outer_loop_count})", border_style="cyan"))
        ...
        with console.status("[bold yellow]AI is coding..."):
            ...
            result = coder_agent.edit_file_content(original_content, coding_instructions, file_path)
        ...
        if new_content == original_content:
            console.print("[yellow]AI made no changes to the file. Skipping review.[/yellow]")
        ...
        while inner_try < max_inner_retries:
            with console.status(f"[bold yellow]Reviewing code changes (Iteration {inner_try + 1}/{max_inner_retries})..."):
                review_result = reviewer.review_code(new_content, task)
            
            console.print(Panel(Markdown(review_result), title=f"Code Review (Iter {outer_loop_count}.{inner_try+1})", border_style="magenta"))
            ...
            if inner_try >= max_inner_retries:
                console.print("[bold red]❌ Maximum review retries reached for this cycle.[/bold red]")
                break

            if Confirm.ask("[bold yellow]Issues found. Attempt auto-fix?[/bold yellow]"):
                with console.status(f"[bold yellow]Fixing code (Attempt {inner_try})..."):
                    fix_result = coder_agent.fix_code(new_content, review_result, feedbacks_history[:-1])
        ...
        if inner_success:
            ...
            summary_feedback = "\n".join([f"Review Iteration {i+1}: {fb}" for i, fb in enumerate(feedbacks_history)])
            feedback_for_planner += f"\n\nMajor Attempt {outer_loop_count} Failed. The code failed the review loop.\nSummary of failures:\n{summary_feedback}"
            if outer_loop_count < MAX_OUTER_LOOP:
                console.print("[bold yellow]Plan failed to produce valid code. Restarting with a new plan...[/bold yellow]")

    if not success:
        console.print(Panel("[bold red]All attempts failed to produce valid code.[/bold red]\nSee output for details.", title="Mission Failed", border_style="red"))
        return
    ...
    if Confirm.ask(f"\n[bold green]Proceed to Commit & Push '{branch_name}'?[/bold green]"):
        ...
        if git_ops.push_changes(branch_name):
            ...
            if Confirm.ask("[bold green]Create Pull Request now?[/bold green]"):
                ...
                if pr_url:
                    console.print(Panel(f"[bold]PR Created Successfully![/bold]\n\n[link={pr_url}]{pr_url}[/link]", title="🚀 Mission Complete", border_style="green"))
                else:
                    console.print("[bold red]Failed to create PR (Check GITHUB_TOKEN).[/bold red]")
        else:
