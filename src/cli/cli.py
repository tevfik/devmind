...
def handle_analyze(args):
    ...
    try:
        ...
        total_files = (
            len([f for f in os.walk(args.path)]) if os.path.isdir(args.path) else 0
        )
        ...
        if args.type == "overview":
            ...
            table = Table(
                title="Repository Overview", show_header=True, header_style="bold cyan"
            )
            ...
            table.add_row(
                "Recent Activity", commits[0]["message"] if commits else "No commits"
            )
            ...
            table = Table(
                title="Project Structure", show_header=True, header_style="bold cyan"
            )
...
"""
            ...
            console.print(
                Panel(panel_text, title="Repository Details", border_style="cyan")
            )

        else:
            # Default: show basic info
            table = Table(
                title="Repository Info", show_header=True, header_style="bold cyan"
            )
...
def handle_simulate(args):
    ...
    try:
        ...
        report = impact_analyzer.analyze_function_change(
            args.function, change_type="signature"
        )
        ...
        analyzer.session.log_finding(
            ...
            else ("MEDIUM" if report["risk_score"] > 30 else "LOW"),
        )
        ...
        if report["direct_callers"]:
            tree = Tree(
                f"[bold]Direct Callers ({len(report['direct_callers'])})[/bold]"
            )
            for caller in report["direct_callers"]:
                tree.add(
                    f"[cyan]{caller['name']}[/cyan] in [dim]{caller['file']}[/dim]"
                )
    ...
    try:
        ...
        if args.type == "call-graph":
            if not args.function:
                console.print(
                    "[red]Error: --function required for call-graph type[/red]"
                )
                return
        ...
        console.print(
            f"[bold green]✅ Visualization saved to:[/bold green] {output_path}"
        )
...
def handle_explain(args):
    ...
    if args.file:
        ...
        try:
            ...
            prompt_input = {"file_name": args.file, "file_content": content}
...
def handle_new(args):
    ...
    if not re.match(r"^[a-zA-Z0-9._-]+$", project_name):
        ...
        print(
            "   Project names can only contain: letters, numbers, dash (-), underscore (_), dot (.)"
        )
        print("\nValid examples:")
        print("  yaver new my_project")
...
"""
    ...
    (path / "tests" / f"test_{name}.py").write_text(
        f"""import pytest
    ...
    try:
        ...
        with adapter.driver.session() as db_session:
            ...
            if not projects:
                ...
                console.print(
                    "[dim]  yaver analyze --type deep <path> --project-id=<name>[/dim]\n"
                )
                return
            ...
            for record in projects:
                ...
                if last_used:
                    ...
                    last_used_dt = datetime.fromtimestamp(
                        last_used / 1000
                    )  # Neo4j timestamp is in milliseconds
...
def handle_project_show(args):
    ...
    try:
        ...
        with adapter.driver.session() as db_session:
            ...
            if not repos:
                console.print(
                    f"[yellow]Project '{project_id}' not found or has no data.[/yellow]\n"
                )
                return
...
def handle_project_delete(args):
    ...
    try:
        ...
        with adapter.driver.session() as db_session:
            ...
            if node_count == 0:
                console.print(
                    f"[yellow]Project '{project_id}' not found or already empty.[/yellow]\n"
                )
                return
        ...
        try:
            ...
            if mem_config.memory_type == "leann":
                ...
                chroma_adapter.delete_by_filter(
                    filter_key="session_id", filter_value=project_id
                )
                ...
                qdrant_adapter.delete_by_filter(
                    filter_key="session_id", filter_value=project_id
                )
...
def handle_project_history(args):
    ...
    try:
        ...
        # Create table
        table = Table(
            title=f"📊 Analysis History: {project_id}",
            show_header=True,
...
def handle_project_cleanup(args):
    ...
    try:
        ...
        history_mgr = ProjectHistoryManager()
        deleted_count = history_mgr.cleanup_old_analyses(
            project_id, keep_last=keep_last
        )

        if deleted_count > 0:
            console.print(f"\n[bold green]✅ Cleanup Complete[/bold green]")
            console.print(
                f"[dim]   Deleted {deleted_count} old analyses for '{project_id}'[/dim]"
            )
...
def handle_agent_analyze(args):
    ...
    try:
        ...
        if not project_root.exists():
            ...
            project_root_alt = (
                Path(os.path.expanduser("~/.yaver/projects")) / args.project_id
            )
            if project_root_alt.exists():
                project_root = project_root_alt
            else:
                console.print(
                    f"[bold red]❌ Error:[/bold red] Project '{args.project_id}' not found in sessions"
                )
                console.print(
                    f"[dim]   Try: yaver analyze <repo> --type deep --project-id {args.project_id}[/dim]\n"
                )
                return
        ...
        if args.format == "json":
            ...
            if issues:
                ...
                for i, issue in enumerate(issues, 1):
                    ...
                    console.print(
                        f"    Priority: {priority}/10 | Effort: ~{effort}h | Risk: {risk}\n"
                    )
            ...
            table = Table(
                title="Code Quality Issues", show_header=True, header_style="bold cyan"
            )
...
def handle_agent_status(args):
    ...
    try:
        agent_state_path = (
            Path(os.path.expanduser("~/.yaver/projects"))
            / args.project_id
            / "agent"
            / "state.json"
        )
...
def handle_agent_history(args):
    ...
    try:
        ...
        table = Table(
            title="Recent Decisions", show_header=True, header_style="bold cyan"
        )
