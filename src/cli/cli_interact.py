    ...
    for match in matches:
        ...
        if target_file:
            try:
                ...
                console.print(f"[yellow]Could not read reference @{file_ref}: {e}[/yellow]")
        else:
...
def handle_suggest(prompt_text: str):
    ...
    try:
        ...
        # UI
        console.print(Panel(Syntax(cmd, "bash"), title="Suggested Command", border_style="green"))
    ...
    while True:
        try:
            ...
            if file_context:
                ...
                console.print(f"[dim green]Included {file_context.count('--- File:')} files in context.[/dim green]")
