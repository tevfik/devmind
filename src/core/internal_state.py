...
class InternalStateManager:
    ...
    def set_current_repo(self, 
                        ...
                        frameworks: Optional[List[str]] = None) -> RepositoryState:
        """
    ...
    def save_state(self):
        ...
        try:
            data = {
                "current_repo_id": self.current_repo.repo_id if self.current_repo else None,
                "version": "1.0",
                "last_updated": datetime.now().isoformat(),
                "repos": {
                    repo_id: state.to_dict()
                    for repo_id, state in self.history.items()
                }
            }
    ...
    def print_current_context(self, detailed: bool = False):
        ...
        repo_table.add_row("Git Commit", f"[magenta]{state.git_commit_hash[:8]}[/magenta]")
        ...
        repo_table.add_row("Last Accessed", state.last_accessed.strftime("%Y-%m-%d %H:%M:%S"))
        ...
        memory_table.add_row("Qdrant Code Collection", f"[cyan]{state.qdrant_collection_code}[/cyan]")
        memory_table.add_row("Qdrant Episodic Collection", f"[cyan]{state.qdrant_collection_episodic}[/cyan]")
        ...
        stats_table.add_row("Memory Size", f"{state.memory_size_bytes / 1024 / 1024:.2f} MB")
