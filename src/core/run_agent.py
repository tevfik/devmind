...
def run_coding_task(task_description: str, output_file: str = None, use_git: bool = False):
    """
    ...
    def cli_handler(event):
        ...
        if event.data:
            if "plan" in event.data:
                console.print(Panel(Markdown(event.data["plan"]), title="Architect's Plan", border_style="blue"))
