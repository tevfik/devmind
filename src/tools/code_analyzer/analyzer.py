...
class CodeAnalyzer:
    ...
    def find_similar_code(
        ...
        try:
            if self.memory_config.memory_type == "leann":
                ...
                vector = self.code_embedder.embed_query(source_code)
                results = self.qdrant_adapter.search(
                    vector, limit=limit, score_threshold=threshold
                )
                return results
        except Exception as e:
    ...
    def analyze_repository(self, incremental: bool = False, use_semantic: bool = False):
            ...
            for file_path in files:
                try:
                    ...
                    if not analysis:
                        ...
                        if parser:
                            ...
                            try:
                                ...
                                logger.warning(
                                    f"Structural parsing failed for {file_path}: {e}"
                                )
                                analysis = None
                        ...
                        if not analysis:
                            ...
                            try:
                                ...
                                logger.warning(
                                    f"Generic parse failed for {file_path}: {e}"
                                )
                                continue
                    ...
                    if (
                        analysis
                        and use_semantic
                        and self.code_embedder
                        and self.qdrant_adapter
                    ):
        ...
        self.session.log_progress(
            f"Completed analysis. Processed {processed_count}/{total_files} files."
        )
        ...
        self.session.log_finding(
            "Analysis Complete", f"Successfully analyzed {processed_count} files."
        )
