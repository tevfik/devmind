...
class LeannAdapter:
    ...
    def __init__(self, session_id: str, config: Optional[Any] = None):
        ...
        logger.debug(f"LeannAdapter initialized for session {session_id} at {self.index_path}")
    ...
    def search(self, query: str, limit: int = 5, score_threshold: float = 0.0, query_filter = None) -> List[Dict[str, Any]]:
        """
        ...
        if not isinstance(query, str):
            logger.warning(f"LeannAdapter expected string query, got {type(query)}. If this is a vector, Leann does not support vector input directly yet.")
            return []
    ...
    def delete_by_filter(self, filter_key: str, filter_value: Any):
        ...
        if filter_key == "session_id" and str(filter_value) == self.session_id:
            ...
            logger.warning(f"LeannAdapter does not support granular deletion by {filter_key} yet. Only session_id handled via folder deletion.")
